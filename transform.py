...
class Transform(object):
    ...
    def __init__(self, keyword, save_path):
        ...
        self.path_raw_data = f'{os.path.expanduser(save_path)}/{keyword}/' \
    ...
    def get_df_grouped_date(self):
        ...
        # Save cleaned data
        save_data = SaveTwitterData(df, self.save_path, self.keyword,
                                    'grouped_date', 'grouped_date', True)
        save_data.save_data()
    ...
    def get_df_key_topics(self, num_tfidf_feat=40):
        ...
        cvec.fit(df.text_clean)
        cvec_counts = cvec.transform(df.text_clean)
        transformed_weights = tfidf.fit_transform(cvec_counts)
        weights = np.asarray(
            transformed_weights.mean(axis=0)).ravel().tolist()
        weights = pd.DataFrame(
            {'topic': cvec.get_feature_names(), 'weight': weights})
        ...
        # Save data
        save_data = SaveTwitterData(df_tfidf, self.save_path, self.keyword,
    ...
    def get_df_cohashtags_matrix(self):
        ...
        # Get co ocurrence matrix by multiplying the one hot encoding by
        # its transpose
