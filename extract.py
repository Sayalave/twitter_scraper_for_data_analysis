...
class Scrape(object):
    ...
    @staticmethod
    def _form_url(keyword_type, since, until, keyword):
        """
        ...
        if keyword_type == 'account':
            return f'https://twitter.com/search?f=tweets&vertical=default&q=' \
                   f'from%3A{keyword}%20since%3A{since}' \
                   f'%20until%3A{until}include%3Aretweets&src=typd'
        elif keyword_type == 'hashtag':
            return f'https://twitter.com/search?f=tweets&vertical=default' \
                   f'&q=(%23{keyword})%20since%3A{since}' \
                   f'%20until%3A{until}include%3Aretweets&src=typed_query'
        elif keyword_type == 'query':
            keyword = keyword.replace("_", '%20')
            return f'https://twitter.com/search?f=tweets&vertical=default' \
                   f'&l=es&q={keyword}%20since%3A{since}' \
                   f'%20until%3A{until}include%3Aretweets&src=typd'
        else:
